--[[
THIS ONE ACTUALLY WORKS detecting if after x time it didn´t accept then cancel trade
]]--
...
local function checkImageTransparency()
    ...
    local acceptTradeCounter = 0

    while acceptTradeCounter <= 15 do
        wait(1)
        acceptTradeCounter = acceptTradeCounter +1
    ...
    if object.ImageTransparency >= 0.5 then
        cancelTrade = true
    end
end
...
while acceptedTrade == false do
    ...
    if checkImageTransparency() then
        print("Accepted: --") -- Print "Accepted" when action is triggered
    end
    
    -- Check if actionTriggered is true outside the function
    if actionTriggered then
        acceptedTrade = true
        print("Action triggered outside the function")
            break
    elseif cancelTrade then
        print("cancel this trade")
